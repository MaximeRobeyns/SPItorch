...
def calculate_maggie_uncertainty(
    mag_error: tensor_like,
    maggie: tensor_like,
) -> tensor_like:
...
def add_maggies_cols(
    input_df: Union[pd.DataFrame, pd.Series], filters: list[Filter]
...
def load_catalogue(
    catalogue_loc: str, filters: list[Filter], compute_maggies_cols: bool = False
) -> pd.DataFrame:
    ...
    maggie_required_cols = (
        [f.maggie_col for f in filters]
        + [f.maggie_error_col for f in filters]
        + ["redshift"]
    )
    mag_required_cols = (
        [f.mag_col for f in filters] + [f.mag_error_col for f in filters] + ["redshift"]
    )
...
def sim_observation(
    filters: list[Filter],
    phot: np.ndarray,
    phot_unc: Optional[np.ndarray] = None,
    index: int = None,
    dset: str = None,
) -> pd.Series:
    ...
    pu = (
        calculate_maggie_uncertainty(phot / 100, phot) if phot_unc is None else phot_unc
    )
...
def filter_has_valid_data(f: Filter, observation: pd.Series) -> bool:
    ...
    valid_value = (
        not pd.isnull(filter_value) and filter_value > -98 and filter_value < 98
    )
    filter_error = observation[f.maggie_error_col]
    assert isinstance(filter_error, np.floating) or isinstance(filter_error, float)
    valid_error = (
        not pd.isnull(filter_error) and filter_error > 0
    )  # <0 if -99 (unknown) or -1 (only upper bound)
    return bool(valid_value and valid_error)


def load_maggies_to_array(
    observation: pd.Series, filters: list[Filter]
...
def load_dummy_observation(
    filter_sel: list[Filter],
) -> tuple[list[observate.Filter], np.ndarray, np.ndarray]:
    ...
    Returns:
        tuple[list[observate.Filter], np.ndarray, np.ndarray]: A list of the
            ...
            uncertainty (ones)
    """
    loaded_filters = observate.load_filters([f.bandpass_file for f in filter_sel])
...
def get_norm_theta_t(
    fp: ForwardModelParams, dtype=None, device=None
...
def save_sim(
    path: str, theta: np.ndarray, cols: list[str], phot: np.ndarray, pw: np.ndarray
):
    ...
    with h5py.File(path, "w") as f:
        ...
        ds_x = grp.create_dataset(
            "theta", data=theta, maxshape=(None, theta.shape[1]), chunks=True
        )
        ds_x.attrs["columns"] = cols
        ds_x.attrs["description"] = "Parameters used by simulator"

        ds_y = grp.create_dataset(
            "simulated_y", data=phot, maxshape=(None, phot.shape[1]), chunks=True
        )
...
def join_partial_results(save_dir: str, n_samples: int, concurrency: int) -> None:
    ...
    with h5py.File(base, "a") as f:
        ...
        for i in range(1, concurrency):
            tmp_f = os.path.join(
                save_dir, f"photometry_sim_{n_samples//concurrency}_{i}.h5"
            )
    ...
    for i in range(1, concurrency):
        os.remove(
            os.path.join(save_dir, f"photometry_sim_{n_samples//concurrency}_{i}.h5")
        )
...
class InMemoryObsDataset(Dataset):
    ...
    def __init__(
        ...
        if os.path.isdir(path):
            ...
            logging.error(
                (
                    f"Provided path ({path}) is neither a directory or "
                    f"path to hdf5 file."
                )
            )
            sys.exit()
    ...
    def __getitem__(
        self, idx: Union[int, list[int], Tensor]
...
class RealObsDataset(Dataset):
    ...
    def _get_x_y_from_file(
        self, path: str, filters: list[Filter]
    ) -> tuple[np.ndarray, np.ndarray]:
