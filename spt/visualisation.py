...
def _get_observer_frame_wavelengths(model: SedModel, sps: SSPBasis) -> np.ndarray:
...
def _get_bounds(
    obs: obs_dict_t,
    wspec: Optional[np.ndarray] = None,
...
def _style_plot(
    fig: plt.Figure, ax: plt.Axes, xmin: float, xmax: float, ymin: float, ymax: float
):
...
def _describe_model(model: SedModel):
    return ", ".join([f"{p}={model.params[p][0]:.4f}" for p in model.free_params])
...
def _add_identity(axes, *line_args, **line_kwargs):
    ...
    cb(axes)
    axes.callbacks.connect("xlim_changed", cb)
    axes.callbacks.connect("ylim_changed", cb)
    return axes

    # https://stackoverflow.com/a/28216751
...
def visualise_obs(
    ...
    fig.text(
        0.06,
        1.007,
        s=f'Filters: {str(obs["filternames"])}',
        fontfamily="sans-serif",
        fontweight="normal",
        fontsize=10,
        alpha=0.7,
...
def visualise_model(
    ...
    b_init = False
    xbounds, ybounds = (0.0, 0.0), (0.0, 0.0)
...
def plot_corner(
    ...
    fig.text(
        0.05, 1.04, s=title, fontfamily="sans-serif", fontweight="demibold", fontsize=25
    )
    fig.text(
        0.05,
        1.005,
...
def plot_posteriors(
    ...
    fig, ax = plt.subplots(
        rows, cols, figsize=(base_size * cols, base_size * rows)
    )  # , dpi=150)
    pltrange = [[0.0, 1.0], [0.0, 1.0]] if lims else None
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            tmp_ax = ax[r][c]
            tmp_ax.hist2d(
                true_ys[:, i], posterior_ys[:, i], bins=50, range=pltrange, cmap="Blues"
            )
            if diags:
                _add_identity(tmp_ax, color="black", ls="--", alpha=0.4)
            tmp_ax.set_xlabel("True Value")
            tmp_ax.set_ylabel("Posterior Samples")
            tmp_ax.set_title(labels[i])

    fig.text(
        0.05, 1.06, s=title, fontfamily="sans-serif", fontweight="demibold", fontsize=25
    )
    fig.text(
        0.05,
        1.005,
...
def ppplot(
    ...
    ax.legend()
    fig.text(
        0.05, 1.04, s=title, fontfamily="sans-serif", fontweight="demibold", fontsize=16
    )
    fig.text(
        0.05,
        1.005,
