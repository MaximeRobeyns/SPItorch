...
\begin{algorithm}[h]
  ...
  \Repeat {$\text{Until reconstructions match the data}$}{
      ...
      $$
      where $\P(\rvz \vert \rvx_{\text{true}}) \propto \P_\rmW(\rvx_{\text{true}}
      \vert \rvz)\P(\rvz)$.\;
  }
  \caption{\label{algo:train}Training Procedure.}
\end{algorithm}

The \acr{ELBO} has the following form:
\begin{align}
    \mathcal{L}(\phi) &= \E_{\Q_{\phi}(\rvz \vert\rvx_{\text{true}})}
        \big[
            \log \P(\rvz, \rvx_{\text{true}}) - \log \Q_\phi(\rvz \vert
            \rvx_{\text{true}})
        \big] \\
                      &= \E_{\Q_{\phi}(\rvz \vert\rvx_{\text{true}})} \left[\log
                      \P_{\rmW}(\rvx_{\text{true}} \vert \rvz) - \log \frac{\Q_\phi(\rvz \vert
            \rvx_{\text{true}})}{\P(\rvz)}\right]
\end{align}
We optimise an \acr{MC} approximation of this objective, using $K$ terms
\[
    \gL(\phi) \approx \sum^K_{i=1}\log \P_\rmW(\rvx_{\text{true}:j}\vert \rvz_i) - \log
    \Q_\phi(\rvz_i \vert \rvx_{\text{true}:j}) + \log \P(\rvz_i),
\]
where $\rvz_i \sim \Q_\phi(\rvz_i \vert \rvx_{\text{true}:j})$.
